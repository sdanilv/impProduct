...
Wayforpay.prototype.getPayUrl = function () {
  if (this.isVerify()) {
    return (
      this.getTargetHost() + "/verify" + (this.skin ? "/" + this.skin : "")
    );
  } else {
...
Wayforpay.prototype.run = function (
  options,
  onApproved,
  onDeclined,
  onPending,
  ...
  this.straightWidget = options.hasOwnProperty("straightWidget")
    ? options.straightWidget
    : false;
  this.makeRedirect = options.hasOwnProperty("makeRedirect")
    ? options.makeRedirect
    : false;
  this.requestType = options.hasOwnProperty("requestType")
    ? options.requestType
    : false;
  this.skin = theme
    ? theme
  ...
  this.isMobile = /Android|webOS|iPhone|iPod|BlackBerry|BB10|IEMobile|Opera Mini/i.test(
    navigator.userAgent
  );
  this.isSafari =
  ...
  if (event.data == "WfpWidgetEventClose") {
    ...
    try {
      ...
      if ("reasonCode" in response) {
        if (
          response.clientStartTime &&
          response.clientStartTime != this.postFields.clientStartTime
        ) {
          return;
        }
...
Wayforpay.prototype.createContainer = function () {
  ...
  if (!style) {
    ...
    cssLink.href = this.getTargetHost() + "/server/pay-widget.min.css?b=51";
...
Wayforpay.prototype.submitData = function (behavior) {
  ...
  form.setAttribute(
    ...
    this.getPayUrl() +
      (behavior
        ? "?behavior=" + behavior + "&build=" + this.build
        : "?build=" + this.build)
  );
